...
!!
!!  This file calculates the diamagnetic spin-orbit coupling integrals using
...
#include "stdout.h"
  ...
  !> \brief calculates the diamagnetic spin-orbit coupling integrals using
  !>        contracted Cartesian Gaussians
  !> \author Bin Gao
  !> \date 2011-09-24
  ...
  !> \param idx_dsorg is the atomic center of nuclear potential origin (<1 for non-atomic center)
  !> \param dso_origin contains the coordinates of nuclear potential origin
  ...
  subroutine contr_cgto_dso(idx_bra, coord_bra, angular_bra, num_prim_bra, &
                               ...
                               idx_dsorg, dso_origin, idx_diporg,         &
    ...
    integer, intent(in) :: idx_dsorg(2)
    real(REALK), intent(in) :: dso_origin(3,2)
...
!f2py intent(in) :: idx_dsorg
!f2py intent(in) :: dso_origin
...
!f2py depend(num_opt) :: contr_ints
    ...
    call dump_gen_opt("dso", 2, (/idx_dsorg,idx_diporg/),      &
                      (/dso_origin,dipole_origin/), scal_const, &
                      (/order_geo_nuc,order_geo_mom/), order_mom,  &
                      order_elec, STDOUT)
#endif
    ! no geometric derivatives
    if (num_cents==0) then
      call contr_cgto_dso_recurr(coord_bra, angular_bra, num_prim_bra,        &
                                    exponent_bra, num_contr_bra, contr_coef_bra, &
      ...
      ! total geometric derivatives of diamagnetic spin-orbit coupling integrals
      if (order_mom==0) then
        ...
        ! we need to scatter the partial and total geometric derivatives into appropriate places
          ...
          if (ierr/=0) stop "contr_cgto_dso>> failed to allocate part_cints/1!"
          ...
          call contr_cgto_dso_recurr(coord_bra, angular_bra, num_prim_bra,         &
                                        ...
                                        dso_origin, dipole_origin, neg_scal_const, &
      ...
      ! total geometric derivatives of diamagnetic spin-orbit coupling with Cartesian
      ! multipole moment integrals
      else
        stop "contr_cgto_dso>> not implemented! please write to authors!"
    ...
    call xtimer_view(curr_time, "contr_cgto_dso", STDOUT)
#endif
    return
  end subroutine contr_cgto_dso

  !> \brief recurrence relations of diamagnetic spin-orbit coupling integrals using
  !>        contracted Cartesian Gaussians
  !> \author Bin Gao
  !> \date 2011-09-24
  !> \param coord_bra contains the coordinates of bra center
  ...
  !> \param num_opt is the number of operators including derivatives
  !> \return contr_ints contains the contracted Cartesian multipole moment integrals
  subroutine contr_cgto_dso_recurr(coord_bra, angular_bra, num_prim_bra,        &
                                      exponent_bra, num_contr_bra, contr_coef_bra, &
                                      coord_ket, angular_ket, num_prim_ket,        &
                                      exponent_ket, num_contr_ket, contr_coef_ket, &
                                      order_geo_bra, order_geo_ket,                &
    ...
    integer, intent(in) :: order_geo_bra
    integer, intent(in) :: order_geo_ket
    real(REALK), intent(in) :: dso_origin(3)
    real(REALK), intent(in) :: dipole_origin(3)
    real(REALK), intent(in) :: scal_const
    integer, intent(in) :: order_geo_nuc
    integer, intent(in) :: order_geo_mom
...
!f2py depend(num_opt) :: contr_ints
    ...
    do jprim = 1, num_prim_ket
      do iprim = 1, num_prim_bra
        ...
        call prim_hgto_dso(order_herm_bra, coord_bra, exponent_bra(iprim), &
...
#if defined(XTIME)
    ! prints the CPU elapsed time
    call xtimer_view(curr_time, "contr_cgto_dso_recurr", STDOUT)
#endif
    return
100 format("contr_cgto_dso_recurr>> ",A,I8,1X,A)
  end subroutine contr_cgto_dso_recurr
