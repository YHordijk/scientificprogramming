...
!!
!!  This file normalizes the contracted Cartesian Gaussians.
!!
!!  2009-10-26, Bin Gao:
!!  * first version
...
#include "stdout.h"
  ...
  !> \brief normalizes the contracted Cartesian Gaussians
  !> \details the normalized contraction coefficient \f$\bar{C}(\alpha_{nl})\f$ is,
  !> \f{eqnarray}{
  !>   \bar{C}(\alpha_{nl})
  !>   &=&\left(\frac{1}{2\pi}\right)^{3/4}(4\alpha_{nl})^{l/2+3/4}
  ...
  !> \f}
  !> where \f$\left(\frac{1}{2\pi}\right)^{3/4}(4\alpha_{nl})^{l/2+3/4}\f$
  ...
  subroutine norm_contr_cgto(angular_num, num_prim, exponents, num_contr, contr_coef)
    use xkind
    implicit none
...
!   real(REALK), parameter :: PREFACT_NRM = sqrt(sqrt((0.5_REALK/PI)**3))
    real(REALK), parameter :: PREFACT_NRM = 0.251979435538381_REALK
                                      !constant \f$\left(\frac{1}{2\pi}\right)^{3/4}\f$
    ...
    do icontr = 1, num_contr
      ...
      do iprim = 1, num_prim
        do jprim = 1, num_prim
          !FIXME: checks something, such as if num_prim==1 .and. exponents<=1.0D-10
      ...
      if (norm_const<NORM_THRSH)                                       &
        call error_stop("norm_contr_cgto",                             &
                        "contraction has zero normalization constant", &
                        icontr)
      ! multiplied by \f$\left(\frac{1}{2\pi}\right)^{3/4}\f$
      norm_const = PREFACT_NRM/norm_const
    ...
    return
  end subroutine norm_contr_cgto
