...
!!
!!  This file calculates the nuclear attraction potential (with Cartesian
...
#include "max_idx_non.h"
  ...
  subroutine contr_cgto_nucpot(idx_bra, coord_bra, angular_bra, num_prim_bra, &
...
!f2py depend(num_opt) :: contr_ints
    ...
    call dump_nucpot(order_elec, idx_nucorg, nucpot_origin, idx_diporg,   &
                     dipole_origin, scal_const, order_mom, order_geo_pot, &
                     order_geo_mom, STDOUT)
#endif
    ! no geometric derivatives
    if (num_cents==0) then
      call contr_cgto_nucpot_recurr(coord_bra, angular_bra, num_prim_bra,        &
                                    exponent_bra, num_contr_bra, contr_coef_bra, &
                                    coord_ket, angular_ket, num_prim_ket,        &
                                    exponent_ket, num_contr_ket, contr_coef_ket, &
                                    order_elec, nucpot_origin, dipole_origin,    &
                                    scal_const, order_mom, order_geo_bra,        &
                                    order_geo_ket, order_geo_pot, order_geo_mom, &
                                    num_cgto_bra, num_cgto_ket, num_opt, contr_ints)
    else
      ! total geometric derivatives of nuclear attraction potential integrals
      if ((order_mom==0 .or. idx_diporg<=MAX_IDX_NON) .and. order_geo_mom==0) then
        ...
        ! we need to scatter the partial and total geometric derivatives into appropriate places
          ...
          dim_geo_mom = (order_geo_mom+1)*(order_geo_mom+2)/2
          allocate(part_cints(dim_cints,num_elec,dim_geo_bra, &
                              dim_geo_ket,dim_geo_pot,dim_geo_mom), stat=ierr)
          if (ierr/=0)                                                              &
            call error_stop("contr_cgto_nucpot", "failed to allocate part_cints/1", &
          ...
          call contr_cgto_nucpot_recurr(coord_bra, angular_bra, num_prim_bra,          &
                                        ...
                                        exponent_ket, num_contr_ket, contr_coef_ket,   &
                                        order_elec, nucpot_origin, dipole_origin,      &
                                        neg_scal_const, order_mom, order_part_cent(1), &
...
!FIXME: order_geo_mom
      ...
      ! total geometric derivatives of nuclear attraction potential with Cartesian
      ! multipole moment integrals
      else
        call error_stop("contr_cgto_nucpot",                        &
    ...
    call xtimer_view(curr_time, "contr_cgto_nucpot", STDOUT)
#endif
    return
  end subroutine contr_cgto_nucpot

  !> \brief recurrence relations of nuclear attraction potential (with Cartesian multipole
  !>        moment) integrals using contracted Cartesian Gaussians
  !> \author Bin Gao
  !> \date 2011-09-24
  !> \param coord_bra contains the coordinates of bra center
  ...
  !> \param num_opt is the number of operators including derivatives
  !> \return contr_ints contains the contracted Cartesian multipole moment integrals
  subroutine contr_cgto_nucpot_recurr(coord_bra, angular_bra, num_prim_bra,        &
                                      exponent_bra, num_contr_bra, contr_coef_bra, &
                                      coord_ket, angular_ket, num_prim_ket,        &
                                      exponent_ket, num_contr_ket, contr_coef_ket, &
                                      order_elec, nucpot_origin, dipole_origin,    &
...
!f2py depend(num_opt) :: contr_ints
    ...
    if (ierr/=0)                                                                   &
      call error_stop("contr_cgto_nucpot_recurr", "failed to allocate hgto_pints", &
    ...
    if (ierr/=0)                                                                       &
      call error_stop("contr_cgto_nucpot_recurr", "failed to allocate cgto_ket_pints", &
    ...
    if (ierr/=0)                                                                   &
      call error_stop("contr_cgto_nucpot_recurr", "failed to allocate cgto_pints", &
    ...
    do jprim = 1, num_prim_ket
      do iprim = 1, num_prim_bra
        ...
        call prim_hgto_nucpot(orders_hgto_bra, coord_bra, exponent_bra(iprim), &
    ...
    if (order_geo_mom==0) then
      ...
      if (ierr/=0)                                                                      &
        call error_stop("contr_cgto_nucpot_recurr", "failed to allocate low_mom_cints", &
...
#if defined(XTIME)
    ! prints the CPU elapsed time
    call xtimer_view(curr_time, "contr_cgto_nucpot_recurr", STDOUT)
#endif
    return
#if defined(DEBUG)
100 format("contr_cgto_nucpot_recurr>> ",A,I8,1X,A)
#endif
  end subroutine contr_cgto_nucpot_recurr
