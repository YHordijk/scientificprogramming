...
#include "max_idx_non.h"
  ...
  subroutine contr_sgto_nucpot(idx_bra, coord_bra, angular_bra, num_prim_bra, &
...
!f2py depend(num_opt) :: contr_ints
    ...
    if (num_cents==0) then
      call contr_sgto_nucpot_recurr(coord_bra, angular_bra, num_prim_bra,        &
                                    exponent_bra, num_contr_bra, contr_coef_bra, &
      ...
      if ((order_mom==0 .or. idx_diporg<=MAX_IDX_NON) .and. order_geo_mom==0) then
        ...
        ! we need to scatter the partial and total geometric derivatives into appropriate places
          ...
          if (ierr/=0)                                                              &
            call error_stop("contr_sgto_nucpot", "failed to allocate part_cints/1", &
...
!FIXME: order_geo_mom
      ...
      ! multipole moment integrals
        ...
        call error_stop("contr_sgto_nucpot",                        &
    ...
    call xtimer_view(curr_time, "contr_sgto_nucpot", STDOUT)
#endif
    return
  end subroutine contr_sgto_nucpot
  ...
  !> \param num_opt is the number of operators including derivatives
  !> \return contr_ints contains the contracted Cartesian multipole moment integrals
  subroutine contr_sgto_nucpot_recurr(coord_bra, angular_bra, num_prim_bra,        &
                                      exponent_bra, num_contr_bra, contr_coef_bra, &
                                      coord_ket, angular_ket, num_prim_ket,        &
                                      exponent_ket, num_contr_ket, contr_coef_ket, &
                                      order_elec, nucpot_origin, dipole_origin,    &
...
!f2py depend(num_opt) :: contr_ints
    ...
    if (ierr/=0)                                       &
      call error_stop("contr_sgto_nucpot_recurr",      &
...
#if defined(XTIME)
    ! prints the CPU elapsed time
    call xtimer_view(curr_time, "contr_sgto_nucpot_recurr", STDOUT)
#endif
    return
#if defined(DEBUG)
100 format("contr_sgto_nucpot_recurr>> ",A,I8,1X,A)
#endif
  end subroutine contr_sgto_nucpot_recurr
