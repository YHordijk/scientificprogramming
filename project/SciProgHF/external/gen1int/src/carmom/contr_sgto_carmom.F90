...
!!  using contracted spherical Gaussians.
!!
!!  2011-03-24, Bin Gao:
!!  * first version
...
#include "stdout.h"
  ...
  subroutine contr_sgto_carmom(idx_bra, coord_bra, angular_bra, num_prim_bra,     &
...
!f2py depend(num_opt) :: contr_ints
    ...
    if (num_cents==0) then
      call contr_sgto_carmom_recurr(coord_bra, angular_bra, num_prim_bra,        &
                                    exponent_bra, num_contr_bra, contr_coef_bra, &
      ...
      if (order_mom==0) then
        ...
        ! we need to scatter the partial and total geometric derivatives into appropriate places
          ...
          if (ierr/=0) stop "contr_sgto_carmom>> failed to allocate part_cints/0!"
      ...
      ! total geometric derivatives of Cartesian multipole moment integrals
        ...
        ! we need to scatter the partial and total geometric derivatives into appropriate places
          ...
          if (ierr/=0) stop "contr_sgto_carmom>> failed to allocate part_cints/1!"
    ...
    call xtimer_view(curr_time, "contr_sgto_carmom", STDOUT)
#endif
    return
  end subroutine contr_sgto_carmom
  ...
  !> \param num_opt is the number of operators including derivatives
  !> \return contr_ints contains the contracted Cartesian multipole moment integrals
  subroutine contr_sgto_carmom_recurr(coord_bra, angular_bra, num_prim_bra,        &
                                      exponent_bra, num_contr_bra, contr_coef_bra, &
                                      coord_ket, angular_ket, num_prim_ket,        &
                                      exponent_ket, num_contr_ket, contr_coef_ket, &
                                      order_elec, dipole_origin, scal_const,       &
...
!f2py depend(num_opt) :: contr_ints
    ...
    integer num_opt_geo         !number of geometric derivatives and operators
    ...
    num_elec_mom = num_low_mom*num_elec
    ...
    if (ierr/=0)                                       &
      call error_stop("contr_sgto_carmom_recurr",      &
    ...
    if (ierr/=0)                                            &
      call error_stop("contr_sgto_carmom_recurr",           &
                      ...
                      dim_hgto_bra*num_hgto_ket*num_elec_mom*num_geo_ket)
    ...
    if (order_geo_bra>0) then
      ...
      if (ierr/=0) stop "contr_sgto_carmom_recurr>> failed to allocate scal_geo_bra!"
        ...
        do jprim = 1, num_prim_ket
          ...
          do iprim = 1, num_prim_bra
            ...
            call prim_hgto_carmom(orders_hgto_bra, coord_bra, exponent_bra(iprim),      &
                                  ...
                                  dim_hgto_bra, dim_hgto_ket, num_elec, num_low_mom,    &
                                  hgto_pints)
            ...
            call scatter_single(dim_hgto_bra, dim_hgto_ket, num_elec_mom, 1, hgto_pints, &
                                angular_ket, order_geo_ket, num_hgto_ket, num_geo_ket,   &
                                kgeo_hgto_pints)
            ! recovers the geometric derivatives on bra center
            call scatter_single(1, dim_hgto_bra, num_hgto_ket*num_elec_mom, num_geo_ket,   &
                                kgeo_hgto_pints, angular_bra, order_geo_bra, num_hgto_bra, &
...
#if defined(XTIME)
    ! prints the CPU elapsed time
    call xtimer_view(curr_time, "contr_sgto_carmom_recurr", STDOUT)
#endif
    return
#if defined(DEBUG)
100 format("contr_sgto_carmom_recurr>> ",A,I8,1X,A)
#endif
  end subroutine contr_sgto_carmom_recurr
