...
#include "stdout.h"
  ...
  subroutine contr_cgto_carmom(idx_bra, coord_bra, angular_bra, num_prim_bra,     &
...
!f2py depend(num_opt) :: contr_ints
    ...
    if (num_cents==0) then
      call contr_cgto_carmom_recurr(coord_bra, angular_bra, num_prim_bra,        &
                                    exponent_bra, num_contr_bra, contr_coef_bra, &
                                    coord_ket, angular_ket, num_prim_ket,        &
                                    exponent_ket, num_contr_ket, contr_coef_ket, &
                                    order_elec, dipole_origin, scal_const,       &
                                    order_mom, order_geo_bra, order_geo_ket,     &
                                    order_geo_mom, num_cgto_bra, num_cgto_ket,   &
                                    num_opt, contr_ints)
    else
      ...
      if (order_mom==0) then
        ...
        ! we need to scatter the partial and total geometric derivatives into appropriate places
          ...
          if (ierr/=0) stop "contr_cgto_carmom>> failed to allocate part_cints/0!"
      ...
      ! total geometric derivatives of Cartesian multipole moment integrals
        ...
        ! we need to scatter the partial and total geometric derivatives into appropriate places
          ...
          if (ierr/=0) stop "contr_cgto_carmom>> failed to allocate part_cints/1!"
          ...
          call contr_cgto_carmom_recurr(coord_bra, angular_bra, num_prim_bra,        &
                                        ...
                                        num_cgto_bra, num_cgto_ket, num_part_opt, part_cints)
          ! scatters the partial and total geometric derivatives
          num_geo_bra = (order_geo_bra+1)*(order_geo_bra+2)/2
          num_geo_ket = (order_geo_ket+1)*(order_geo_ket+2)/2
          num_geo_mom = (order_geo_mom+1)*(order_geo_mom+2)/2
          num_tot_geo = num_opt/(num_elec_mom*num_geo_bra*num_geo_ket*num_geo_mom)
    ...
    call xtimer_view(curr_time, "contr_cgto_carmom", STDOUT)
#endif
    return
  end subroutine contr_cgto_carmom
  ...
  !> \param num_opt is the number of operators including derivatives
  !> \return contr_ints contains the contracted Cartesian multipole moment integrals
  subroutine contr_cgto_carmom_recurr(coord_bra, angular_bra, num_prim_bra,        &
                                      exponent_bra, num_contr_bra, contr_coef_bra, &
                                      coord_ket, angular_ket, num_prim_ket,        &
                                      exponent_ket, num_contr_ket, contr_coef_ket, &
                                      order_elec, dipole_origin, scal_const,       &
...
!f2py depend(num_opt) :: contr_ints
    ...
    if (ierr/=0)                                       &
      call error_stop("contr_cgto_carmom_recurr",      &
    ...
    if (order_geo_mom==0) then
      ...
      !                            num_elec,num_mom,num_geo_bra,num_geo_ket)
...
#if defined(XTIME)
    ! prints the CPU elapsed time
    call xtimer_view(curr_time, "contr_cgto_carmom_recurr", STDOUT)
#endif
    return
#if defined(DEBUG)
