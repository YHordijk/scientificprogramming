...
!!  using contracted spherical Gaussians, but transformed back from
...
#include "stdout.h"
  ...
  !>        contracted spherical Gaussians, but transformed back from
  !>        contracted Cartesian Gaussians
  !> \author Bin Gao
  !> \date 2011-10-07
  ...
  subroutine contr_csgto_carmom(idx_bra, coord_bra, angular_bra, num_prim_bra,     &
                                ...
                                order_elec, idx_diporg, dipole_origin, scal_const, &
...
!f2py depend(num_opt) :: contr_ints
    integer idx_part_cent(3)    !indices of bra, ket and dipole origin centers
    integer order_part_cent(3)  !orders of partial geometric derivatives of bra, ket and dipole origin centers
    ...
    integer seq_part_geo(3)     !sequence of operator, bra and ket centers for partial geometric derivatives
    ...
    integer dim_geo_mom         !dimension of geometric derivatives on operator center before scattering
    ...
    integer num_geo_mom         !number of partial geometric derivatives on operator center after scattering
    integer num_tot_geo         !number of total geometric derivatives after scattering
    real(REALK), allocatable :: part_cints(:,:,:,:,:)  !contains the derivatives of integrals before scattering
    ...
    call dump_carmom(order_elec, idx_diporg, dipole_origin, scal_const, &
                     order_mom, order_geo_mom, STDOUT)
#endif
    ! no geometric derivatives
    if (num_cents==0) then
      call contr_csgto_carmom_recurr(coord_bra, angular_bra, num_prim_bra,        &
                                     exponent_bra, num_contr_bra, contr_coef_bra, &
      ...
      ! total geometric derivatives of overlap integrals
      if (order_mom==0) then
        ...
        ! we need to scatter the partial and total geometric derivatives into appropriate places
          ...
          allocate(part_cints(dim_cints,num_elec,dim_geo_bra,dim_geo_ket,1), stat=ierr)
          if (ierr/=0) stop "contr_csgto_carmom>> failed to allocate part_cints/0!"
          num_part_opt = num_elec*dim_geo_bra*dim_geo_ket
          call contr_csgto_carmom_recurr(coord_bra, angular_bra, num_prim_bra,        &
                                         exponent_bra, num_contr_bra, contr_coef_bra, &
                                         coord_ket, angular_ket, num_prim_ket,        &
                                         exponent_ket, num_contr_ket, contr_coef_ket, &
                                         order_elec, dipole_origin, scal_const, 0,    &
                                         order_part_cent(1), order_part_cent(2),      &
                                         order_geo_mom, num_sgto_bra, num_sgto_ket,   &
                                         num_part_opt, part_cints)
          ...
          num_tot_geo = num_opt/(num_elec*num_geo_bra*num_geo_ket)
          call geom_part_zero_scatter(num_cents, order_cent, seq_part_geo(1:num_cents), &
                                      order_geo_bra, order_geo_ket, dim_cints*num_elec, &
                                      dim_geo_bra, dim_geo_ket, 1, part_cints,          &
                                      num_geo_bra, num_geo_ket, num_tot_geo, contr_ints)
      ...
      ! total geometric derivatives of Cartesian multipole moment integrals
      else
        ! sets the orders of partial geometric derivatives, the last is dipole origin
        idx_part_cent(1) = idx_bra
        idx_part_cent(2) = idx_ket
        idx_part_cent(3) = idx_diporg
        order_part_cent(1) = order_geo_bra
        order_part_cent(2) = order_geo_ket
        order_part_cent(3) = order_geo_mom
        ...
        if (zero_ints .or. order_part_cent(3)>order_mom) then
          contr_ints = 0.0_REALK
        ! we need to scatter the partial and total geometric derivatives into appropriate places
        else if (scatter_deriv) then
          ! allocates memory for the derivatives of integrals before scattering
          ...
          num_elec_mom = num_mom*num_elec
          dim_geo_bra = (order_part_cent(1)+1)*(order_part_cent(1)+2)/2
          dim_geo_ket = (order_part_cent(2)+1)*(order_part_cent(2)+2)/2
          dim_geo_mom = (order_part_cent(3)+1)*(order_part_cent(3)+2)/2
          allocate(part_cints(dim_cints,num_elec_mom,dim_geo_bra, &
                              dim_geo_ket,dim_geo_mom), stat=ierr)
          if (ierr/=0) stop "contr_csgto_carmom>> failed to allocate part_cints/1!"
          ...
          call contr_csgto_carmom_recurr(coord_bra, angular_bra, num_prim_bra,        &
                                         ...
                                         coord_ket, angular_ket, num_prim_ket,        &
                                         exponent_ket, num_contr_ket, contr_coef_ket, &
                                         order_elec, dipole_origin, neg_scal_const,   &
                                         order_mom, order_part_cent(1),               &
                                         order_part_cent(2), order_part_cent(3),      &
                                         num_sgto_bra, num_sgto_ket, num_part_opt, part_cints)
          ! scatters the partial and total geometric derivatives
          num_geo_bra = (order_geo_bra+1)*(order_geo_bra+2)/2
          num_geo_ket = (order_geo_ket+1)*(order_geo_ket+2)/2
          num_geo_mom = (order_geo_mom+1)*(order_geo_mom+2)/2
          num_tot_geo = num_opt/(num_elec_mom*num_geo_bra*num_geo_ket)
          call geom_part_one_scatter(num_cents, order_cent, seq_part_geo(1:num_cents), &
                                     order_geo_bra, order_geo_ket, order_geo_mom,      &
                                     dim_cints*num_elec_mom, dim_geo_bra, dim_geo_ket, &
                                     dim_geo_mom, 1, part_cints, num_geo_bra,          &
                                     num_geo_ket, num_geo_mom, num_tot_geo, contr_ints)
          deallocate(part_cints)
        else
          ! sets the scale constant
          if (neg_one) then
    ...
    call xtimer_view(curr_time, "contr_csgto_carmom", STDOUT)
#endif
    return
  end subroutine contr_csgto_carmom
  ...
  !> \param num_opt is the number of operators including derivatives
  !> \return contr_ints contains the contracted Cartesian multipole moment integrals
  subroutine contr_csgto_carmom_recurr(coord_bra, angular_bra, num_prim_bra,        &
                                       exponent_bra, num_contr_bra, contr_coef_bra, &
                                       coord_ket, angular_ket, num_prim_ket,        &
                                       exponent_ket, num_contr_ket, contr_coef_ket, &
                                       order_elec, dipole_origin, scal_const,       &
...
!f2py depend(num_contr_ket) :: contr_ints
!f2py depend(num_opt) :: contr_ints
    integer num_cgto_bra        !number of Cartesian Gaussians on bra center
    integer num_cgto_ket        !number of spherical Gaussians on ket center
    ...
    call xtimer_set(curr_time)
#endif
    ! computes the number of Cartesian Gaussians
    num_cgto_bra = (angular_bra+1)*(angular_bra+2)/2
    ...
    num_opt_geo = num_elec_mom*num_geo_bra*num_geo_ket
    ! allocates the memory for primitive integrals
    allocate(hgto_pints(dim_hgto_bra,dim_hgto_ket,num_elec_mom), stat=ierr)
    if (ierr/=0)                                       &
      call error_stop("contr_csgto_carmom_recurr",     &
                      "failed to allocate hgto_pints", &
                      dim_hgto_bra*dim_hgto_ket*num_elec_mom)
    allocate(cgto_ket_pints(dim_hgto_bra,num_cgto_ket, &
                            num_elec_mom,num_geo_ket), stat=ierr)
    if (ierr/=0)                                           &
      call error_stop("contr_csgto_carmom_recurr",         &
                      "failed to allocate cgto_ket_pints", &
                      dim_hgto_bra*num_cgto_ket*num_elec_mom*num_geo_ket)
    allocate(cgto_pints(num_cgto_bra,num_cgto_ket,num_opt_geo, &
                        num_prim_bra,num_prim_ket), stat=ierr)
    if (ierr/=0)                                       &
      call error_stop("contr_csgto_carmom_recurr",     &
                      "failed to allocate cgto_pints", &
    ...
    do jprim = 1, num_prim_ket
      do iprim = 1, num_prim_bra
        ...
        call prim_hgto_carmom(orders_hgto_bra, coord_bra, exponent_bra(iprim),      &
                              orders_hgto_ket, coord_ket, exponent_ket(jprim),      &
                              order_elec, dipole_origin, scal_const, order_low_mom, &
                              dim_hgto_bra, dim_hgto_ket, num_elec, num_low_mom, hgto_pints)
        ...
        call hgto_to_cgto(angular_ket, order_geo_ket, exponent_ket(jprim), &
                          dim_hgto_bra, dim_hgto_ket, num_elec_mom, 1,     &
        ...
        call hgto_to_cgto(angular_bra, order_geo_bra, exponent_bra(iprim),         &
                          1, dim_hgto_bra, num_cgto_ket*num_elec_mom, num_geo_ket, &
                          cgto_ket_pints, num_cgto_bra, num_geo_bra,               &
                          cgto_pints(:,:,:,iprim,jprim))
      end do
    end do
    ! cleans
    ...
    if (ierr/=0)                                          &
      call error_stop("contr_csgto_carmom_recurr",        &
                      ...
                      *num_cgto_ket*num_contr_ket*num_opt_geo)
#if defined(DEBUG)
    write(STDOUT,100)                                                             &
    ...
    allocate(sgto_ket_cints(num_cgto_bra,num_contr_bra, &
    ...
    write(STDOUT,100) "size of contracted SGTO (on ket center) integrals:", &
                      size(sgto_ket_cints)
#endif
    ! transforms the CGTOs to SGTOs on ket center
    call hgto_to_sgto_oop(angular_ket, num_cgto_bra*num_contr_bra, &
                          num_cgto_ket, num_contr_ket*num_opt_geo, &
                          low_mom_cints, num_sgto_ket, sgto_ket_cints)
    deallocate(low_mom_cints)
    ! transforms the HGTOs to SGTOs on bra center
    if (order_geo_mom==0) then
      call hgto_to_sgto_oop(angular_bra, 1, num_cgto_bra,                         &
      ...
      call carmom_deriv(order_low_mom, size(low_mom_cints)/num_opt_geo*num_elec, &
                        num_low_mom, num_geo_bra*num_geo_ket, low_mom_cints,     &
                        order_mom, order_geo_mom, (order_mom+1)*(order_mom+2)/2, &
...
#if defined(XTIME)
    ! prints the CPU elapsed time
    call xtimer_view(curr_time, "contr_csgto_carmom_recurr", STDOUT)
#endif
    return
#if defined(DEBUG)
100 format("contr_csgto_carmom_recurr>> ",A,I8,1X,A)
#endif
  end subroutine contr_csgto_carmom_recurr
