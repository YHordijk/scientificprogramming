...
#include "stdout.h"
  ...
  subroutine contr_cgto_gaupot(idx_bra, coord_bra, angular_bra, num_prim_bra,      &
...
!f2py depend(num_opt) :: contr_ints
    ...
    if (num_cents==0) then
      call contr_cgto_gaupot_recurr(coord_bra, angular_bra, num_prim_bra,        &
                                    exponent_bra, num_contr_bra, contr_coef_bra, &
      ...
      if (order_mom==0) then
        ! sets the orders of partial geometric derivatives, the last is potential origin
        ...
        ! we need to scatter the partial and total geometric derivatives into appropriate places
          ...
          if (ierr/=0)                                                              &
            call error_stop("contr_cgto_gaupot", "failed to allocate part_cints/1", &
          ...
          call contr_cgto_gaupot_recurr(coord_bra, angular_bra, num_prim_bra,        &
                                        ...
                                        order_part_cent(3), 0, num_cgto_bra,         &
                                        num_cgto_ket, num_part_opt, part_cints)
    ...
    call xtimer_view(curr_time, "contr_cgto_gaupot", STDOUT)
#endif
    return
  end subroutine contr_cgto_gaupot
  ...
  !> \param num_opt is the number of operators including derivatives
  !> \return contr_ints contains the contracted Cartesian multipole moment integrals
  subroutine contr_cgto_gaupot_recurr(coord_bra, angular_bra, num_prim_bra,        &
                                      exponent_bra, num_contr_bra, contr_coef_bra, &
                                      coord_ket, angular_ket, num_prim_ket,        &
                                      exponent_ket, num_contr_ket, contr_coef_ket, &
                                      order_elec, gaupot_origin, gaupot_expt,      &
...
!f2py depend(num_opt) :: contr_ints
    ...
    if (ierr/=0)                                                                   &
      call error_stop("contr_cgto_gaupot_recurr", "failed to allocate hgto_pints", &
    ...
    if (ierr/=0)                                                                       &
      call error_stop("contr_cgto_gaupot_recurr", "failed to allocate cgto_ket_pints", &
    ...
    if (ierr/=0)                                                                   &
      call error_stop("contr_cgto_gaupot_recurr", "failed to allocate cgto_pints", &
    ...
    if (order_geo_mom==0) then
      ...
      if (ierr/=0)                                                                      &
        call error_stop("contr_cgto_gaupot_recurr", "failed to allocate low_mom_cints", &
...
#if defined(XTIME)
    ! prints the CPU elapsed time
    call xtimer_view(curr_time, "contr_cgto_gaupot_recurr", STDOUT)
#endif
    return
#if defined(DEBUG)
100 format("contr_cgto_gaupot_recurr>> ",A,I8,1X,A)
#endif
  end subroutine contr_cgto_gaupot_recurr
