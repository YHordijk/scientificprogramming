...
!!
!!  Tests subroutines in nucpot_hket.F90.
!!
!!  2011-10-18, Bin Gao
!!  * first version

  !> \brief tests subroutines in nucpot_hket.F90
  !> \author Bin Gao
  !> \date 2011-10-18
  !> \param io_log is the IO unit of log file
  !> \return test_failed indicates if the test is failed
  subroutine test_nucpot_hket(io_log, test_failed)
    use xkind
...
#include "test_f90/prim_gto.h"
    ...
    ! the following tests are for individual cases in \fn(nucpot_hket)
    orders_hgto_ket = (/0,0/)
    orders_geo_pot = (/0,6/)
    call sub_test_nucpot_hket(orders_hgto_ket, orders_geo_pot, ORDER_ELEC, &
                              io_log, test_failed)
    orders_hgto_ket = (/0,1/)
    orders_geo_pot = (/0,6/)
    call sub_test_nucpot_hket(orders_hgto_ket, orders_geo_pot, ORDER_ELEC, &
    ...
    call sub_test_nucpot_hket(orders_hgto_ket, orders_geo_pot, ORDER_ELEC, &
                              io_log, test_failed)
    return
  end subroutine test_nucpot_hket

  !> \brief tests subroutines in nucpot_hket.F90 with specific orders of
  ...
  !> \param io_log is the IO unit of log file
  !> \return test_failed indicates if the test is failed
  subroutine sub_test_nucpot_hket(orders_hgto_ket, orders_geo_pot, order_elec, &
                                  io_log, test_failed)
    use xkind
    ! module of HTML test log routines
    use html_log
...
#include "test_f90/prim_gto.h"
    ...
    real(REALK) recur_pint          !results from recursive function \fn(recur_nucpot_hket)
    ...
    logical different               !if result from \fn(nucpot_hket) is different from reference
    ! dumps information of tests
    ...
    max_order_nucpot = orders_hgto_ket(2)+orders_geo_pot(2)
    ...
    if (orders_geo_pot(1)>0) then
      dim_geo_hket = ((orders_geo_pot(2)+1)*(orders_geo_pot(2)+2) &
                      *(orders_geo_pot(2)+3)                      &
    ...
    allocate(hket_pints(dim_hgto_ket,dim_geo_hket), stat=ierr)
    if (ierr/=0) then
      call html_log_int_number(log_text="Failed to allocate hket_pints:",        &
                               log_int=dim_hgto_ket*dim_geo_hket, fmt_int="I12", &
                               io_log=io_log, font_color="red")
      deallocate(nucpot_pints)
      test_failed = .true.
      return
    end if
    call nucpot_hket(orders_hgto_ket, orders_geo_pot, COORD_BRA, EXPONENT_BRA, &
                     COORD_KET, EXPONENT_KET, dim_nucpot, nucpot_pints,        &
    ...
    call html_log_real_number(                                            &
           log_text="Time (s) used for nucpot_geom and nucpot_hket:",     &
    ...
    ! initializes the address of integrals from \fn(nucpot_hket)
    addr_geo = 0
    ...
    do order_geo = orders_geo_pot(1), orders_geo_pot(2)
      ...
      do z_geo = 0, order_geo
        do y_geo = 0, order_geo-z_geo
          ...
          ! initializes the address of integrals from \fn(nucpot_hket)
          addr_hket = 0
          ! loops over the orders of HGTOs
          do order_hket = orders_hgto_ket(1), orders_hgto_ket(2)
            ! loops over the xyz components of HGTOs
            do z_hket = 0, order_hket
              do y_hket = 0, order_hket-z_hket
                x_hket = order_hket-(z_hket+y_hket)
                ! integral from \fn(recur_nucpot_hket)
                recur_pint = recur_nucpot_hket((/x_hket,y_hket,z_hket/), &
                                               (/x_geo,y_geo,z_geo/))
                addr_hket = addr_hket+1
                call check_difference(recur_pint, hket_pints(addr_hket,addr_geo), &
                                      different)
                if (different) then
                  ...
                  call html_log_int_array(                               &
                         ...
                         io_log=io_log)
                  call html_log_real_number(                           &
                         log_text="Reference from recur_nucpot_hket:", &
                         log_real=recur_pint, fmt_real="Es20.12",      &
                         io_log=io_log, font_color="blue")
                  call html_log_real_number(log_text="Result from nucpot_hket:",     &
                                            log_real=hket_pints(addr_hket,addr_geo), &
    ...
    call xtimer_set(end_time)
    ! prints the CPU elapsed time
    call html_log_real_number(log_text="Time (s) used for recur_nucpot_hket:", &
                              log_real=end_time-begin_time, fmt_real="F10.4",  &
                              io_log=io_log, font_color="blue")
    ! cleans
    deallocate(hket_pints)
    return
  end subroutine sub_test_nucpot_hket
