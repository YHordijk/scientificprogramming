...
#Note that these variables are set by pam (after defining them in .diracrc), this runscript is needed if you don't use pam.

#ExaTensor specific:
export QF_PATH=../..              #full path to ExaTENSOR root directory
...
#/usr/local/mpi/openmpi/3.1.0/bin/mpiexec -n $QF_NUM_PROCS -npernode $QF_PROCS_PER_NODE -oversubscribe ./exacorr.x #>& exacorr.log

#/usr/local/mpi/mpich/3.2.1/bin/mpiexec -n $QF_NUM_PROCS ./exacorr.x #>& exacorr.log
#/usr/local/mpi/mpich/3.2.1/bin/mpiexec -n $QF_NUM_PROCS -hosts localhost ./exacorr.x #>& exacorr.log  #For Mac users
#mpiexec -n $QF_NUM_PROCS -hosts localhost ./exacorr.x 2>&1 | tee exacorr.log  #For Mac users

#aprun -n $QF_NUM_PROCS -N $QF_PROCS_PER_NODE -d $QF_CORES_PER_PROCESS -cc none ./exacorr.x #>& exacorr.log

#jsrun --smpiargs="-mca common_pami_use_odp 1" -D PAMI_IBV_DISABLE_ODP=0 -n $QF_NUM_PROCS -r $QF_PROCS_PER_NODE -a 1 -c $QF_CORES_PER_PROCESS -g $QF_GPUS_PER_PROCESS -bnone ./exacorr.x #>& exacorr.log
#jsrun --smpiargs="-mca common_pami_use_odp 1" -D PAMI_IBV_DISABLE_ODP=0 -n $QF_NUM_PROCS -r $QF_PROCS_PER_NODE -a 1 -c $QF_CORES_PER_PROCESS -g $QF_GPUS_PER_PROCESS -bnone nvprof -o trace.%q{OMPI_COMM_WORLD_RANK} ./exacorr.x #>& exacorr.log
