...
!This module contains an in-core Hartree-Fock implementation
    ...
    subroutine hartree_fock_driver(oneint,twoint)
        ...
        do i = 1, nspin
            D(i,i,1) = 1
        enddo
        call calculate_energy(oneint%h_core, twoint, D, energy, set)
    ...
    subroutine calculate_energy(h, g, D, energy, set)
        complex(8), intent(in) :: h(:,:), g(:,:,:,:)
        real(8), intent(in) :: D(:,:,:)
        ...
        do p = 1, n
            do q = 1, n
                if(set%UHF) then
                    energy1 = energy1 + h((p-1)/2+1, (q-1)/2+1) * CD(p,q)
                else
